...
var condata={type:2,title:"收款记录",rimg:btnImg,hback:handback}
...
request_http.on("success", function(databus) {
	...
	if(databus.length!=undefined){
		...
		for(var i=0;i<databus.length;i++){
			var _data = databus[i];
			data1[data1.length] = {"gtxt":_data.date,"nums":_data.number+"条(合计:"+_data.amount+"元)"};
			var _data2 = [];
...
function init(n){
	...
	expData1.removeAll();
	expData2.removeAll();
	request_http.url = $U.token($U.url.merchantPaymentRecords,"token")+"&type="+n;
