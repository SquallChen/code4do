...
var newdata = {
	"photo" : "source://image/movies/Masters_of_Sex.png",
	"name" : "Masters of Sex",
...
http.on("success", function(data) {
	...
	if(!info){
		root.add("info",
                 "source://view/custom_head_foot_view/refresh_notification.ui",
				 0, 128);
		info = ui("info");
	}else{
		info.visible = true;
...
listview.on("push", function(data) {
	...
	if (data.state == 2) {
		if (!added) {
			initdata.readFile("initdata://moremovie.json", function(
					data, e) {
				listdata.addData(data);
				listview.refreshItems();
				added = true;
