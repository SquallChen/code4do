...
root.on("switchAzt",function(data){
	deviceone.off = data[1];
	deviceone.on = data[2];
	swbox.tag = data[0];
	root.fire("switchAtag",swbox.tag);//fire
	if(swbox.tag==0){
		swbtn.text = data[1];
		swbtn.x = 0; 
		...
		swbtn.text = data[2];
		swbtn.x =50;
		swbtn.fontColor = "408080FF";
		swbg.bgColor = "408080FF";
	}
	swbtn.width = 80;
	swbtn.height = 80;
	swbtn.y = 10;
...
swbox.on("touch",function(){
	...
	var propscoloryes = {bgColor:"408080FF"};
	...
	root.fire("switchAtag",swbox.tag);
});